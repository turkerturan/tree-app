...
export const selectRootNodeList = (state: RootState) => state.tree.nodeList.filter((item: any) => item.root)

export function getNodeSelector(id: string) {
    return (state: RootState) => state.tree.nodeList.find((item: any) => item.id === id)
}
